...
class App extends Component {
  state = {isActiveTab: tabsList[0].tabId}

  updateTabItems = value => {
  ...
  filteredList = () => {
    ...
    const filteredProjects = projectsList.filter(
      eachId => eachId.category === isActiveTab,
    )
        ...
        <ul className="tabs-container">
          {tabsList.map(tabDetails => (
            <TabItem
              ...
              updateTabItem={this.updateTabItems}
              isActive={tabDetails.tabId === isActiveTab}
            />
          ))}
        </ul>

        <ul className="project-list-container">
          {filteredItems.map(projectDetails => (
            <ProjectItem
